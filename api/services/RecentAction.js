const RecentAction = module.exports = {
  generator: {
    ...
    taskReply: function (studentId, taskId, labgroupId, cb) {
      Users.findOne(studentId).exec((err, student) => {
        if (err) return cb(err)
        LabGroups.findOne(labgroupId).exec((err, labgroup) => {
          if (err) return cb(err)
          Tasks.findOne(taskId).populate('topic').exec((err, task) => {
            if (err) return cb(err)
            cb(null,`Student ${student.fullName()} wysłał odpowiedź do zadania <a href='/teacher/replies/view/${task.id}'>[${task.topic.number}. ${task.topic.title}/${task.number}. ${task.title}]</a> w grupie laboratoryjnej ${labgroup.name}`)
          })
        })
      })
    }
  ...
  sendTaskReply: function (studentId, taskId, labgroupId, cb){
    RecentTeacherActions.findOrCreate({student: studentId, labgroup: labgroupId, task: taskId, seen: false, type:2},{student: studentId, labgroup: labgroupId, task: taskId, type:2})
      .exec((err,action,created)=>{
        if(err) return cb(err)
        if(created){
          this.generator.taskReply(studentId, taskId, labgroupId, (err,msg)=>{
            if (err) return cb(err)
  ...
  addStudentComment: function (studentId, taskId, labgroupId, cb){
    RecentTeacherActions.findOrCreate({student: studentId, labgroup: labgroupId, task: taskId, seen: false, type:1},{student: studentId, labgroup: labgroupId, task: taskId, type:1})
      .exec((err,action,created)=>{
      if(err) return cb(err)
      if(created){
